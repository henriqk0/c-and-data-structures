...
t_veiculo *loadveics(char *nomearq) {
  ...
  // if preventivo caso o nome do arquivo inexista ou seja alterado por alguma
  // razão
  if (arq == NULL) {
...
void filtro(t_veiculo *vet, char *marca_veiculo, int tam_vet) {
  ...
  for (int i = 0; i < tam_vet; i++) {
    ...
    // retira o "\n" do final de vet[i].marca para tornar-se comparável com
    // marca_veiculo
    if (vet[i].marca[strlen(vet[i].marca) - 1] == '\n')
      vet[i].marca[strlen(vet[i].marca) - 1] = '\0';

    // verifica se as strings são iguais (não sabia que podia strcmp)
    ...
    if (iguais == indexStr) {
      ...
      // escreve no novo arquivo e, por fim, pula uma linha para o próximo
      // veículo
